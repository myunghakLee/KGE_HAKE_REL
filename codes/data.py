...
class TrainDataset(Dataset):
    def __init__(self, data_reader: DataReader, neg_size: int, batch_type: BatchType):
        ...
        self.hr_map, self.tr_map, self.ht_map, self.hr_freq, self.tr_freq, self.ht_freq = self.two_tuple_count()
    ...
    def __getitem__(self, idx: int):
        ...
        while neg_size < self.neg_size:
            if self.batch_type == BatchType.REL_BATCH:
                neg_triples_tmp = np.random.randint(self.num_relation, size=self.neg_size * 2)
    ...
    def two_tuple_count(self):
        ...
        for head, rel, tail in self.triples:
            ...
            if (head, tail) not in ht_map.keys():
                ht_map[(head, tail)] = set()
            ...
            if (head, tail) not in ht_freq.keys():
                ht_freq[(head, tail)] = init_cnt

            hr_map[(head, rel)].add(tail)
            tr_map[(tail, rel)].add(head)
            ht_map[(head, tail)].add(rel)
        ...
        return hr_map, tr_map, ht_map, hr_freq, tr_freq, ht_freq
...
class BidirectionalOneShotIterator(object):
    def __init__(self, dataloader_head, dataloader_tail, dataloader_rel):
        self.iterator_head = self.one_shot_iterator(dataloader_head)
        self.iterator_tail = self.one_shot_iterator(dataloader_tail)
        self.iterator_rel = self.one_shot_iterator(dataloader_rel)
