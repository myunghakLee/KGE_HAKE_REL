...
class KGEModel(nn.Module, ABC):
    ...
    def forward(self, sample, batch_type=BatchType.SINGLE):
        ...
        if batch_type == BatchType.SINGLE:
            ...
            gt_part, rel_part = sample
            batch_size, negative_sample_size = rel_part.size(0), rel_part.size(1)
                ...
                for positive_sample, negative_sample, filter_bias, batch_type in test_dataset:
                    ...
                    for i in range(batch_size):
                        ...
                        logs.append({
                            ...
                            'HITS@40': 1.0 if ranking <= 40 else 0.0,
                            'HITS@50': 1.0 if ranking <= 50 else 0.0,
                            'HITS@60': 1.0 if ranking <= 60 else 0.0,
                            'HITS@70': 1.0 if ranking <= 70 else 0.0,
                            'HITS@80': 1.0 if ranking <= 80 else 0.0,
                            'HITS@90': 1.0 if ranking <= 90 else 0.0,
                            'HITS@95': 1.0 if ranking <= 95 else 0.0,
                            'HITS@96': 1.0 if ranking <= 96 else 0.0,
                        })
